                ...
                <div class="table-wrapper">
                    <div class="table-title">
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <a href="#addDataModal" class="btn btn-success" data-toggle="modal"><i
                ...
                <div class="modal-content">
                    <form id="updateDataForm">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Device ID</label>
                                <!-- Sesuaikan dengan atribut yang ingin Anda izinkan untuk diedit -->
                                <input type="text" id="update_device_id" name="device_id" class="form-control"
                                    required>
                            </div>
                            <div class="form-group">
                                <label>Value</label>
                                <input type="number" id="update_value" name="value" class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label>Max Value</label>
                                <input type="number" id="update_max_value" name="max_value" class="form-control"
                                    required>
                            </div>
                            <div class="form-group">
                                <label>Min Value</label>
                                <input type="number" id="update_min_value" name="min_value" class="form-control"
                                    required>
                            </div>
                        </div>
                        <div class="modal-footer">
        ...
        <script>
            $(document).ready(function() {
                ...
                function fetchDatas() {
                    axios.get('/api/datas')
                        .then(response => {
                            ...
                            let dataTableBody = $('#dataTableBody');
                            dataTableBody.empty();
                            data.forEach(data => {
                        ...
                        <tr>
                            <td>
                                <span class="custom-checkbox">
                                    <input type="checkbox" id="selectAll${data.id}" name="options[]" value="${data.id}">
                                    <label for="checkbox${data.id}"></label>
                                </span>
                            </td>
                            <td><a href="/api/data/${data.id}" class="data-detail">${data.id}</a></td>
                            <td>${data.device_id}</td>
                            <td>${data.value}</td>
                            <td>${data.max_value}</td>
                            <td>${data.min_value}</td>
                            <td>
                                <a href="#editDataModal" class="edit" data-toggle="modal" onclick="editData(${data.id})"><i
                                        class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a>
                                <a href="#deleteDataModal" class="delete" data-toggle="modal" onclick="deleteData(${data.id})"><i
                ...
                window.editData = function(id) {
                    axios.get(`/api/data/${id}`)
                        .then(response => {
                            ...
                            $('#update_device_id').val(data.device_id);
                            $('#update_value').val(data.value);
                            $('#update_max_value').val(data.max_value);
                            $('#update_min_value').val(data.min_value);
                        })
                        .catch(error => {
                            console.log(error);
                        });
                ...
                $('#updateDataForm').submit(function(event) {
                    ...
                    let formData = $(this).serialize();
                    axios.put(`/api/data/${id}`, formData)
                        .then(response => {
                            alert(response.data.message);
                            fetchdata();
                ...
                window.deleteData = function(id) {
                    $('#deleteDataForm').submit(function(event) {
                        event.preventDefault();
